...
When built for Shopify app proxy deployment, the static frontend must use
`VITE_MESH_API_BASE_URL=/tools/mesh-splitter/api`. Browser requests to `/api`
hit the Shopify storefront root and return 404.
